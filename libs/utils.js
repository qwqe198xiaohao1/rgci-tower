...
utils.prototype.formatBigNumber = function (x, digits) {
    ...
    // 如果数字小于10000，显示为整数
    if (absX < 10000) {
        return sign + Math.round(absX).toString();
    }
     ...
     var units = [];
    for (var exponent = 1; exponent <= 308; exponent++) {
        // 注意：1e308可能会超过JavaScript的最大安全整数，但作为数值比较是可以的
        units.push({ "val": Math.pow(10, exponent), "suffix": "e" + exponent });
    }
    ...
    for (var i = units.length - 1; i >= 0; i--) {
        var each = units[i];
        if (absX >= each.val) {
            var scaled = absX / each.val;
            return sign + scaled.toFixed(2) + each.suffix;
        }
    }
    
    return sign + absX.toFixed(2);
}
