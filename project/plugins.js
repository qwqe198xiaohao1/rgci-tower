var plugins_bb40132b_638b_4a9f_b028_d3fe47acc8d1 = 
{
	...
	setInterval(function () {
		...
		if (core.actions._checkReplaying && core.actions._checkReplaying()) return;
		if (core.status.lockControl) return;
		if (core.status.hero.exp <= 0) {
			core.status.hero.hp += 1;
			core.status.hero.exp += Math.floor(Math.pow(10, Math.pow(core.status.hero.hp, core.getFlag('lvzs', 1)))) / 5;
		}
		if (core.getFlag("jcjy") <= 0) {
			core.setFlag("jc", core.getFlag("jc") + 1);
			core.setFlag("jcjy", core.getFlag("jcjy") + Math.floor(Math.pow(10, Math.pow(core.getFlag("jc"), core.getFlag('jczs', 1)))));
		}
		core.updateStatusBar(true);
	...
	setInterval(function () {
		...
		//基础
		core.setFlag('lvpzy', Math.max(Math.floor(Math.pow(core.status.hero.hp - 29, core.getFlag('pexp', 1))), 0));
		core.setFlag('lvsjzy', Math.max(Math.floor(Math.pow(Math.max((core.status.hero.hp - 97), 0) / 3, core.getFlag('sjexp', 0.5))), 0));
		core.setFlag('pzsjzy', Math.max(Math.floor(Math.pow(Math.max(Math.log10(Math.max(core.status.hero.def / 2048, 1)) / Math.log10(2), 0), core.getFlag('sjexp', 0.5))), 0));
		//同步
		core.setFlag('zczy', Math.floor(core.getFlag('gczy', 1) * Math.max(core.getFlag('pczy', 1), 1) * Math.pow(core.getFlag('gcfw', 1), 2) * Math.max(core.getFlag('bczy', 1), 1) * Math.max(core.getFlag('sjczy', 1), 1)));
		core.setFlag('zjyzy', core.getFlag('gjyzy', 1) * Math.max(core.getFlag('pjyzy', 1), 1) * Math.max(core.getFlag('bjyzy', 1), 1) * Math.max(core.getFlag('sjjyzy', 1), 1) * Math.max(core.getFlag('jceff', 1), 1));
		core.setFlag('zpzy', Math.max(Math.max(core.getFlag('lvpzy', 1), 1) * Math.max(core.getFlag('ppzy', 1), 1) * Math.max(core.getFlag('bjpzy', 1), 1) * Math.max(core.getFlag('sjpzy', 1), 1)), 1);
		core.setFlag('zbjzy', Math.max(core.getFlag('pbjzy', 1), 1) * Math.max(core.getFlag('sjbjzy', 1), 1));
		core.setFlag('zsjzy', Math.max(Math.max(core.getFlag('lvsjzy', 1), 0) * Math.max(core.getFlag('pzsjzy', 1), 0) * Math.max(core.getFlag('sjsjzy', 1), 1), 1), 0);
		core.setFlag('zjczy', core.getFlag('gjczy', 1) * Math.max(core.getFlag('pjczy', 1), 1) * Math.max(core.getFlag('sjjczy', 1), 1));
		core.updateStatusBar(true);
	}, 50);
}
}